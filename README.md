...
现在的下单接口已经是一个生产级可用的接口了，可以用于中小型规模的秒杀，还可以继续引入一些机制来完善。

## 引入死信队列

虽然Spring有自带的消费重试机制，但如果因为某种原因就是消费不了这个消息，就会被直接丢弃，这在某些对于消息可靠性要求较高的业务场景下，显然不太合适了。 因此可以引入死信队列，用于兜底处理异常订单补偿 / 人工处理这些异常订单。

有两种方式可以引入死信交换机/队列。

- 在注解上直接配置 ackMode=MANUAL，自己编写相应逻辑；
- 使用Spring提供的重试次数耗尽的消息处理策略，如果主流程连续失败 3 次（配置的次数），消息会进入 DLQ；

## 引入支付服务

现在的服务是没有支付服务的，可以引入支付服务更加真实，记住标准电商都是“`先下单，再支付`”。所以引入支付服务后的流程如下：
...
生成预订单 ID（Snowflake）
   ↓
MQ 发送消息（order.create）
   ↓
立刻返回订单号给前端（此时状态 = "待支付"）
   ↓
前端显示支付按钮（倒计时15分钟）
   ↓
用户点击 → 调用支付服务（支付宝/微信）下单接口
   ↓
支付成功（微信/支付宝回调支付服务）
   ↓
支付服务发送“支付成功消息”给订单服务（或RPC）
   ↓
订单服务改状态 → 已支付
```
...
秒杀一般要求“下单后 15 分钟未支付自动取消”。流程：

1. 创建订单后写入订单表（状态：UNPAID）
2. 同时发送一个延时消息（15 分钟）
3. 消费者收到延时消息检查是否已经支付
   - 未支付 → 取消订单 + 恢复库存
   - 已支付 → 忽略

如果用 RabbitMQ，延时队列可以用 TTL + DLX 或 x-delayed-message 插件实现。
...
支付网关（支付宝/微信）
  ↓
支付成功回调 PaymentService
  ↓
PaymentService 通过 MQ/RPC 通知订单服务
  ↓
订单服务设置订单状态（PAID）
...
检查订单状态
  ↓
如果 UNPAID → 取消订单 + 恢复库存（MySQL + Redis）
```

延迟消息实现方式：

- TTL + DLX：给队列或消息设置 TTL，消息 TTL 到期后会变成 死信（Dead Letter），死信会自动转发到指定的 死信交换机（DLX）。
- x-delayed-message 插件：消息不会进入死信，而是交换机内部计时，到时间后再投递。

> 延时消息不会导致支付确认延迟，只在订单未支付时才执行取消动作。它是一个兜底操作：假如用户未支付，则自动取消订单 + 恢复库存。
